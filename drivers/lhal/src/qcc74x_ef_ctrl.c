...
void ATTR_TCM_SECTION qcc74x_ef_ctrl_write_common_trim(struct qcc74x_device_s *dev, char *name, uint32_t value, uint8_t program)
{
    ...
    for (i = 0; i < trim_list_len; i++) {
        if (arch_memcmp(name, trim_list[i].name, qcc74x_ef_ctrl_strlen(name)) == 0) {
            ...
            putreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].en_addr / 32) * 4, reg_val);

            parity = qcc74x_ef_ctrl_get_trim_parity(value, trim_list[i].value_len);
            if (parity) {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].parity_addr / 32) * 4);
                reg_val |= (1 << (trim_list[i].parity_addr % 32));
                putreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].parity_addr / 32) * 4, reg_val);
            }

            if (((trim_list[i].value_addr % 32) + trim_list[i].value_len) > 32) {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
                reg_val |= (value << (trim_list[i].value_addr % 32));
                putreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4, reg_val);

                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4) + 4;
                reg_val |= (value >> (32 - (trim_list[i].value_addr % 32)));
                putreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4 + 4, reg_val);
            } else {
                reg_val = getreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4);
                reg_val |= (value << (trim_list[i].value_addr % 32));
                putreg32(QCC74x_EF_CTRL_BASE + (trim_list[i].value_addr / 32) * 4, reg_val);

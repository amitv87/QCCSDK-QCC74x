...
#else
extern void qcc74x_bootrom_printf(char *fmt, ...);
#define SDIO2_DRV_ERR(a, ...) qcc74x_bootrom_printf("[Error]:" a, ##__VA_ARGS__)
#endif
...
#if defined(QCC743)
static void qcc74x_sdio2_reset_irq(int irq, void *arg);
#endif

/* dnld irq callback */
static void *sdio2_dnld_irq_callback_arg;
static qcc74x_sdio2_trans_irq_callback_t sdio2_dnld_irq_callback;
/* upld irq callback */
static void *sdio2_upld_irq_callback_arg;
static qcc74x_sdio2_trans_irq_callback_t sdio2_upld_irq_callback;
/* error irq callback */
static void *sdio2_error_irq_callback_arg;
static qcc74x_sdio2_error_irq_callback_t sdio2_error_irq_callback;

#if defined(QCC743)
/* reset irq callback */
static void *sdio2_reset_irq_callback_arg;
static qcc74x_sdio2_reset_irq_callback_t sdio2_reset_irq_callback;
#endif
...
/* dnld ring queue */
static volatile qcc74x_sdio2_trans_desc_t sdio_dnld_port_table[SDIO2_MAX_PORT_NUM];
static volatile uintptr_t sdio_dnld_port_head, sdio_dnld_port_tail;

/* upld ring queue */
static volatile qcc74x_sdio2_trans_desc_t sdio_upld_port_table[SDIO2_MAX_PORT_NUM];
...
int qcc74x_sdio2_dnld_port_push(struct qcc74x_device_s *dev, qcc74x_sdio2_trans_desc_t *trans_desc)
{
    ...
    if (trans_desc->buff_len < qcc74x_sdio2_get_dnld_max_size(dev)) {
...
int qcc74x_sdio2_upld_port_push(struct qcc74x_device_s *dev, qcc74x_sdio2_trans_desc_t *trans_desc)
{
    ...
    if (trans_desc->data_len > SDIO2_BYTE_MOD_SIZE_MAX) {
        ...
        trans_len = (trans_desc->data_len + qcc74x_sdio2_get_block_size(dev) - 1);
        trans_len -= trans_len % (qcc74x_sdio2_get_block_size(dev));
    } else {
...
int qcc74x_sdio2_init(struct qcc74x_device_s *dev, uint32_t dnld_size_max)
{
    ...
    qcc74x_sdio2_trans_desc_t trans_desc_null = { 0 };
...
#if defined(QCC743)
#define QCC743_IRQ_SDU_SOFT_RST 19
    qcc74x_irq_attach(QCC743_IRQ_SDU_SOFT_RST, qcc74x_sdio2_reset_irq, dev);
    qcc74x_irq_enable(QCC743_IRQ_SDU_SOFT_RST);
#endif
...
int qcc74x_sdio2_get_dnld_size_max(struct qcc74x_device_s *dev)
{
    return sdio2_dnld_size_max;
}

int qcc74x_sdio2_get_upld_size_max(struct qcc74x_device_s *dev)
{
    return sdio2_upld_size_max;
}
    ...
    if ((regval & SDIO2_CCR_CIC_UpLdOvr)) {
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio_upld_port_head == sdio_upld_port_tail) {
                ...
                /*  */
                qcc74x_sdio2_trans_desc_t upld_trans_desc = sdio_upld_port_table[sdio_upld_port_head & SDIO2_MAX_PORT_MASK];
    ...
    if ((regval & SDIO2_CCR_CIC_DnLdOvr) &&
        ...
        for (uint8_t i = 0; i < SDIO2_MAX_PORT_NUM; i++) {
            if (sdio_dnld_port_head == sdio_dnld_port_tail) {
                ...
                /*  */
                qcc74x_sdio2_trans_desc_t trans_desc = sdio_dnld_port_table[sdio_dnld_port_head & SDIO2_MAX_PORT_MASK];
...
static void qcc74x_sdio2_reset_irq(int irq, void *arg)
{
    ...
    qcc74x_irq_clear_pending(QCC743_IRQ_SDU_SOFT_RST);

    uint32_t dnld_desc_num = 0;
    qcc74x_sdio2_trans_desc_t dnld_desc_s[SDIO2_MAX_PORT_NUM] = { 0 };

    uint32_t upld_desc_num = 0;
    qcc74x_sdio2_trans_desc_t upld_desc_s[SDIO2_MAX_PORT_NUM] = { 0 };

...
{
#ifdef romapi_qcc74x_sha_init
    romapi_qcc74x_sha_init(dev, ctx);
#else
    uint32_t regval;
    uint32_t reg_base;
...
{
#ifdef romapi_qcc74x_sha_init
    return romapi_qcc74x_sha_init(dev, ctx, input, len);
#else
                           ...
                           uint8_t *output)
{
    ...
    romapi_qcc74x_sha1_link_finish(dev, ctx, output);
#else
    uint32_t last, padn;
    uint32_t high, low;
    uint8_t msgLen[8];
...
void qcc74x_group0_request_sha_access(struct qcc74x_device_s *dev)
{
    ...
    romapi_qcc74x_group0_request_sha_access(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
void qcc74x_group0_release_sha_access(struct qcc74x_device_s *dev)
{
    ...
    romapi_qcc74x_group0_release_sha_access(dev);
#else
    uint32_t reg_base;

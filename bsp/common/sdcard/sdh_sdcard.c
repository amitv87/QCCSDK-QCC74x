...
static status_t SD_SelectCard(sd_card_t *card, QCC74x_Fun_Type NewState);
...
{
    //qcc74x_platform_printf("Interrupt occurs! intFlag=0x%02x,\r\n",handle->intFlag);
...
static status_t SDH_INT_Init(void)
{
    qcc74x_irq_attach(33, sdh_isr, NULL);
    qcc74x_irq_enable(33);
...
static status_t SDH_SendCardCommand(SDH_CMD_Cfg_Type *cmd)
{
    ...
    while (SDH_CMDWaitStatus == SD_WAITING) {
        if ((uint32_t)qcc74x_mtimer_get_time_ms() - time_node > SDIO_CMDTIMEOUT_MS) {
            SDH_MSG("SDH send CMD%ld timeout: %ld ms\r\n", cmd->index, (uint32_t)qcc74x_mtimer_get_time_ms() - time_node);
    ...
    while (1) {
        ...
        } else if ((uint32_t)qcc74x_mtimer_get_time_ms() - time_node > SDIO_CMDTIMEOUT_MS) {
            SDH_MSG("SDH send CMD%ld timeout: %ld ms\r\n", cmd->index, (uint32_t)qcc74x_mtimer_get_time_ms() - time_node);
            return Status_Timeout;
        }
        QCC74x_DRV_DUMMY;
...
static status_t SD_SendScr(sd_card_t *card)
{
    ...
    while (SDH_DataWaitStatus == SD_WAITING) {
        if ((uint32_t)qcc74x_mtimer_get_time_ms() - time_node > SDIO_CMDTIMEOUT_MS) {
            SDH_MSG("sdh get csr data timeout: %ld ms\r\n", (uint32_t)qcc74x_mtimer_get_time_ms() - time_node);
...
static status_t SDH_CardTransferNonBlocking(SDH_DMA_Cfg_Type *dmaCfg, SDH_Trans_Cfg_Type *transfer)
{
    ...
    qcc74x_l1c_dcache_clean_range((void *)(dmaCfg->admaEntries), dmaCfg->maxEntries * sizeof(SDH_ADMA2_Desc_Type));
...
status_t SDH_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    /* QCC74x_undef/QCC74x_undef supports only 8-byte aligned addresses */
...
#if 0
    qcc74x_l1c_dcache_clean_range((void *)(readbuff), 1);
    qcc74x_l1c_dcache_clean_range((void *)(readbuff + BlockSize * NumberOfBlocks), 1);
    qcc74x_l1c_dcache_invalidate_range((void *)(readbuff), BlockSize * NumberOfBlocks);
#else
    qcc74x_l1c_dcache_clean_range((void *)(readbuff), BlockSize * NumberOfBlocks);
#endif
    ...
    if (pdTRUE != xSemaphoreTake(SDH_DataWaitSem, SDIO_CMDTIMEOUT_MS)) {
        SDH_MSG("SDH read data timeout: %ld", (uint32_t)qcc74x_mtimer_get_time_ms() - time_node);
...
status_t SDH_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    qcc74x_l1c_dcache_clean_range((void *)(writebuff), BlockSize * NumberOfBlocks);

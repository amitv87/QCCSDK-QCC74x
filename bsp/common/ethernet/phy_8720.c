...
int phy_8720_reset(void)
{
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_SPECIAL_MODES, &regval)) {
    ...
    if (PHY_8720_SPECIAL_MODES_MODE_PWRDOWN == (regval & PHY_8720_SPECIAL_MODES_MODE)) {
        if (qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_SPECIAL_MODES, regval | PHY_8720_SPECIAL_MODES_MODE_ALL) != 0) {
            return -1;
        }
    }

    /* disable energy detect powerdown mode for cable detect, this increase the power by 220mW */
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_CTRL_STATUS, &regval)) {
        return -1;
    }

    if (qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_CTRL_STATUS, regval & (~PHY_8720_CTRL_STATUS_EDPWRDOWN)) != 0) {
        return -1;
    }

    /* do sw reset */
    if (qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_BCR, PHY_8720_RESET) != 0) {
        return -1;
    }

    for (; timeout; timeout--) {
        if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_BCR, &regval)) {
            return -1;
        }
...
int phy_8720_auto_negotiate(struct qcc74x_emac_phy_cfg_s *cfg)
{
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_PHYID1, &phyid1)) {
        printf("read emac phy id 1 error\r\n");
        return -1;
    }

    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_PHYID2, &phyid2)) {
    ...
    if (qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_BCR, regval) != 0) {
        return -1;
    }
    ...
    if (qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_ADVERTISE, advertise) != 0) {
        return -1;
    }
    ...
    while (1) {
        if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_BSR, &regval)) {
            return -1;
        }
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_LPA, &lpa)) {
        return -1;
    }
...
int phy_8720_link_up(struct qcc74x_emac_phy_cfg_s *cfg)
{
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_BSR, &phy_bsr)) {
        return -1;
    }
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_SR, &phy_sr)) {
        return -1;
    }
...
int phy_8720_poll_cable_status(struct qcc74x_emac_phy_cfg_s *cfg)
{
    ...
    if (0 != qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_CTRL_STATUS, &phy_regval)) {
        return -1;
    }
...
int phy_8720_init(struct qcc74x_device_s *emac, struct qcc74x_emac_phy_cfg_s *cfg)
{
    ...
    // qcc74x_emac_phy_set_address(emac_dev, cfg->phy_address);
    ...
    if (cfg->auto_negotiation) {
        ...
        uint32_t cnt=0;
        do{
            if(qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_BSR, &phyReg) != SUCCESS){
                return -1;
            }
            cnt++;
        ...
        */
        ...
        if (qcc74x_emac_phy_reg_read(emac_dev, PHY_8720_BCR, &phyReg) != 0) {
            return -1;
        }
        ...
        if ((qcc74x_emac_phy_reg_write(emac_dev, PHY_8720_BCR, phyReg)) != 0) {
            return -1;
        }
    }

    // qcc74x_emac_phy_config_full_duplex(emac_dev, cfg->full_duplex);

...
This demo mainly introduces IR to send and receive data using the rc5 protocol.
...
For more detailed code, please refer to **examples/peripherals/ir/ir_rc5**
...
- Get ``irtx`` handle
- Set tx_mode to RC5 mode and call ``qcc74x_ir_tx_init(irtx, &tx_cfg)`` to initialize ir tx
...
- Get ``irrx`` handle
- Set rx_mode to RC5 mode and call ``qcc74x_ir_rx_init(irrx, &rx_cfg)`` to initialize ir rx

...
void emac_isr(int irq, void *arg)
{
    ...
    if (int_sts_val & EMAC_INT_STS_RX_DONE) {
        ...
        uint32_t rx_len;
        qcc74x_emac_bd_rx_dequeue(-1, &rx_len, NULL);
...
int main(void)
{
    ...
    qcc74x_emac_bd_init(emac0, (uint8_t *)ethTxBuff, ETH_TXBUFNB, (uint8_t *)ethRxBuff, ETH_RXBUFNB);
    ...
    while (1) {
        ...
        qcc74x_emac_bd_tx_enqueue(EMAC_NORMAL_PACKET, sizeof(test_frame), test_pattern);
        test_pattern[27]++;
        qcc74x_emac_bd_rx_dequeue(EMAC_NORMAL_PACKET, &rx_len, NULL);
        qcc74x_emac_bd_tx_enqueue(EMAC_NORMAL_PACKET, sizeof(test_frame), test_pattern);
        test_pattern[27]++;
        qcc74x_emac_bd_tx_enqueue(EMAC_NORMAL_PACKET, sizeof(test_frame), test_pattern);
        test_pattern[27]++;
        ...
        loop++;
        if ((loop & 0xfffff) == 0) {
            time = qcc74x_mtimer_get_time_ms();

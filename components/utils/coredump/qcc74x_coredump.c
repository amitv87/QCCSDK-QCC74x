...
/* 1KB Block */
#define QCC74x_COREDUMP_PRINT_SEG_N_K 1
...
//uint32_t _$coredump_binary_id$_ __attribute__((used, section(".coredump_binary_id"))) = QCC74x_COREDUMP_BINARY_ID;
...
qcc74x sdk not for qcc74x_undef wifi */
#elif defined(QCC74x_undef)
    // {(uintptr_t)0x28008000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD, "EM_REG"},
...
/* #elif defined(QCC74x_undef)
    {(uintptr_t)0xf0004000, (unsigned int)12, DUMP_CSR, "dump riscv csr"},
    {(uintptr_t)0x28010000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD, "EM_REG"},
qcc74x sdk not for qcc74x_undef */
#elif defined(QCC743)
    {(uintptr_t)0xf0004000, (unsigned int)12, DUMP_CSR, "dump riscv csr"},
    // {(uintptr_t)0x28010000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD, "EM_REG"},
#endif
...
static void cd_flush(void) {
  //qcc74x_uart_flush(COREDUMP_UART);
}

static int cd_getchar(char *inbuf) {
  int ret;
  ret = qcc74x_uart_getchar(console);
  if (ret < 0) {
    return 0;
  }
  *inbuf = (char)ret;
...
static void cd_putchar(const char *buf, size_t len) {
  size_t i;
  for (i=0; i<len; i++) {
    qcc74x_uart_putchar(console, buf[i]);
  }
}
 ...
 * @return result
 */
static void qcc74x_coredump_print(uintptr_t addr, uint32_t len, const char *desc, enum dump_type type) {
  struct crc32_stream_ctx crc_ctx;
  uint32_t crc;
...
#ifndef QCC74x_COREDUMP_PRINT_SEG_N_K
#define QCC74x_COREDUMP_PRINT_SEG_N_K 32
#endif
static void qcc74x_coredump_print_n_k(uintptr_t addr, uint32_t len, const char *desc, enum dump_type type)
{
    uint32_t printed_len;
    uint32_t seg;

    for(seg = 0; seg * QCC74x_COREDUMP_PRINT_SEG_N_K * 1024 < len; seg++) {
        printed_len = seg*QCC74x_COREDUMP_PRINT_SEG_N_K*1024;
        qcc74x_coredump_print(addr + printed_len, len - printed_len >= QCC74x_COREDUMP_PRINT_SEG_N_K*1024 ? QCC74x_COREDUMP_PRINT_SEG_N_K*1024: len - printed_len, desc, type);
    }
}
 ...
 * @return result
 */
void qcc74x_coredump_parse(const uint8_t *buf, unsigned int len) {
  char command;
  unsigned int i = 0;
  ...
  case 'x':
    do {
      ...
      if (len - i >= 8) {
        ...
        qcc74x_coredump_print_n_k(addr, length, NULL, DUMP_BASE64_WORD);
      }
    } while (0);
    return;
  ...
  case 'd':
    do {
      for (i = 0; i < (sizeof(mem_hdr) / sizeof(mem_hdr[0])); i++) {
        qcc74x_coredump_print_n_k(mem_hdr[i].addr, mem_hdr[i].length, mem_hdr[i].desc, mem_hdr[i].type);
      }
    } while (0);
    return;
...
void qcc74x_coredump_run() {
  ...
  for (cmd_pos = 0; cmd_pos < (sizeof(mem_hdr) / sizeof(mem_hdr[0])); cmd_pos++) {
    ...
    qcc74x_coredump_print_n_k(mem_hdr[cmd_pos].addr, mem_hdr[cmd_pos].length, mem_hdr[cmd_pos].desc, mem_hdr[cmd_pos].type);
  }
  for (cmd_pos = 0; (&_dump_sections + cmd_pos)->addr != 0xffffffff; cmd_pos++) {
    ...
    if ((&_dump_sections + cmd_pos)->len == 0)
      continue;
    qcc74x_coredump_print_n_k((&_dump_sections + cmd_pos)->addr, (&_dump_sections + cmd_pos)->len, "predefined", DUMP_BASE64_WORD);
  }
  ...
  _reactive: /* In some scenarios we need to reactivate the state machine */
    ...
    case COREDUMP_PARSE:
      status = COREDUMP_IDLE;
      qcc74x_coredump_parse(cmd_buf, cmd_pos);
      continue;
    }
  }
}
